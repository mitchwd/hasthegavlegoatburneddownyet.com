...
const createUTCDate = (year: number, month: number, day: number): Date =>
  new Date(Date.UTC(year, month - 1, day, 9, 0, 0)); // 9am UTC

describe(getFirstSundayOfAdvent, () => {
  it('when Christmas is on a Saturday', () => {
    const firstSundayOfAdvent = getFirstSundayOfAdvent(2021);
    expect(firstSundayOfAdvent).toStrictEqual(
      new Date('2021-11-28T00:00:00.000Z'),
    );
    expect(
      new Intl.DateTimeFormat('sv-SE', { weekday: 'long' }).format(
        firstSundayOfAdvent,
      ),
    ).toBe('söndag');
    expect(
      new Intl.DateTimeFormat('en-AU', { weekday: 'long' }).format(
        firstSundayOfAdvent,
      ),
    ).toBe('Sunday');
  });

  it('when Christmas is on a Sunday', () => {
  ...
  it('when Christmas is on a Monday', () => {
    const firstSundayOfAdvent = getFirstSundayOfAdvent(2023);
    expect(firstSundayOfAdvent).toStrictEqual(
      new Date('2023-12-03T00:00:00.000Z'),
    );
    expect(
      new Intl.DateTimeFormat('sv-SE', { weekday: 'long' }).format(
        firstSundayOfAdvent,
  ...
  it('on a leap year (2020)', () => {
    const firstSundayOfAdvent = getFirstSundayOfAdvent(2020);
    expect(firstSundayOfAdvent).toStrictEqual(
      new Date('2020-11-29T00:00:00.000Z'),
    );
    expect(
      new Intl.DateTimeFormat('sv-SE', { weekday: 'long' }).format(
        firstSundayOfAdvent,
...
describe('getFirstWeekdayAfterNewYear', () => {
  it('when New Years Day is on a Friday, should return Monday', () => {
    const nextWeekday = getFirstWeekdayAfterNewYearsDay(2021);
    expect(nextWeekday).toStrictEqual(new Date('2021-01-04T00:00:00.000Z'));
    expect(
      new Intl.DateTimeFormat('sv-SE', { weekday: 'long' }).format(nextWeekday),
    ).toBe('måndag');
    expect(
      new Intl.DateTimeFormat('en-AU', { weekday: 'long' }).format(nextWeekday),
    ).toBe('Monday');
  });

  it('when New Years Day is on a Saturday, should return Monday', () => {
    const nextWeekday = getFirstWeekdayAfterNewYearsDay(2022);
    expect(nextWeekday).toStrictEqual(new Date('2022-01-03T00:00:00.000Z'));
    expect(
      new Intl.DateTimeFormat('sv-SE', { weekday: 'long' }).format(nextWeekday),
    ).toBe('måndag');
    expect(
  ...
  it('when New Years Day is on a Sunday, should return Tuesday', () => {
    const nextWeekday = getFirstWeekdayAfterNewYearsDay(2023);
    expect(nextWeekday).toStrictEqual(new Date('2023-01-03T00:00:00.000Z'));
    expect(
      new Intl.DateTimeFormat('sv-SE', { weekday: 'long' }).format(nextWeekday),
    ).toBe('tisdag');
    expect(
  ...
  it('when New Years Day is on a Monday, should return Tuesday', () => {
    const nextWeekday = getFirstWeekdayAfterNewYearsDay(2024);
    expect(nextWeekday).toStrictEqual(new Date('2024-01-02T00:00:00.000Z'));
    expect(
      new Intl.DateTimeFormat('sv-SE', { weekday: 'long' }).format(nextWeekday),
    ).toBe('tisdag');
    expect(
  ...
  it('when New Years Day is on a Tuesday, should return Wednesday', () => {
    const nextWeekday = getFirstWeekdayAfterNewYearsDay(2030);
    expect(nextWeekday).toStrictEqual(new Date('2030-01-02T00:00:00.000Z'));
    expect(
      new Intl.DateTimeFormat('sv-SE', { weekday: 'long' }).format(nextWeekday),
    ).toBe('onsdag');
    expect(
  ...
  it('when New Years Day is on a Wednesday, should return Thursday', () => {
    const nextWeekday = getFirstWeekdayAfterNewYearsDay(2025);
    expect(nextWeekday).toStrictEqual(new Date('2025-01-02T00:00:00.000Z'));
    expect(
      new Intl.DateTimeFormat('sv-SE', { weekday: 'long' }).format(nextWeekday),
    ).toBe('torsdag');
    expect(
  ...
  it('when New Years Day is on a Thursday, should return Friday', () => {
    const nextWeekday = getFirstWeekdayAfterNewYearsDay(2026);
    expect(nextWeekday).toStrictEqual(new Date('2026-01-02T00:00:00.000Z'));
    expect(
      new Intl.DateTimeFormat('sv-SE', { weekday: 'long' }).format(nextWeekday),
    ).toBe('fredag');
    expect(
...
describe(isDateInAdventSeason, () => {
  it('should return true for First Sunday of Advent', () => {
  ...
  it('should return false for day before First Sunday of Advent', () => {
  ...
  it('should return true for last day of Advent season', () => {
  ...
  it('should return false for day after last day of Advent season', () => {
  ...
  it("should handle New Year's Day on Sunday correctly", () => {
  ...
  it("should handle New Year's Day on Friday correctly", () => {
