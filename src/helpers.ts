export function getFirstSundayOfAdvent(year: number): Date {
  ...
  firstSundayBeforeChristmas.setUTCDate(
    christmas.getUTCDate() - daysToFirstSunday,
  );
  ...
  fourthSundayBeforeChristmas.setUTCDate(
    firstSundayBeforeChristmas.getUTCDate() - 3 * 7,
  );
...
export function isDateInAdventSeason(date: Date): boolean {
  ...
  const firstWeekdayAfterNewYear = getFirstWeekdayAfterNewYearsDay(
    adventYear + 1,
  );
  ...
  const normalizedStart = new Date(
    Date.UTC(
      firstSundayOfAdvent.getUTCFullYear(),
      firstSundayOfAdvent.getUTCMonth(),
      firstSundayOfAdvent.getUTCDate(),
    ),
  );
  const normalizedEnd = new Date(
    Date.UTC(
      firstWeekdayAfterNewYear.getUTCFullYear(),
      firstWeekdayAfterNewYear.getUTCMonth(),
      firstWeekdayAfterNewYear.getUTCDate(),
    ),
  );
